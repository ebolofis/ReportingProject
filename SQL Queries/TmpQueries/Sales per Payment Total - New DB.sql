...
--------------------------------------------------------------------
-- Πωλήσεις ανά Χειριστη και Τμήμα, POS, Τρόπο Πληρωμής Συνοπτικά --
--------------------------------------------------------------------
...
From OrderDetailInvoices odi 
     ...
     left join ProductCategories pc on pc.Id = case odi.IsExtra when 1 then odi.productCategoryId
     ...
     inner join (
                ...
                Group by InvoicesId
                       , AccountId
                       , a.Type
                       , a.Description ) as t on t.InvoicesId = i.Id

Where IsNull(i.IsDeleted, 0) = 0
      and IsNull(odi.IsDeleted, 0) = 0
      and InvoiceType not in (2,8)
...
Group By ProductId
       ...
       , DepartmentDescription
       , DepartmentId
       -- , PricelistCode, PricelistDescription, InvoiceTotal, InvoiceType, Abbreviation, ReceiptNo
       , Cover
       , AccountDescription
       , AccountType
       , AccountId
