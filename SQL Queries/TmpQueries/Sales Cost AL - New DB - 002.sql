...
FROM OrderDetailInvoices odi
     LEFT OUTER JOIN EndOfDay eod ON ISNULL(eod.FODay, CAST(GETDATE() AS DATE)) BETWEEN CAST(@FromDate AS DATE)
                                                                                    AND CAST(@ToDate AS DATE)
     INNER JOIN Invoices i ON i.Id = odi.InvoicesId
                              AND IsNull(i.IsVoided, 0) = 0
                              and ISNULL(i.EndOfDayId,0) = CASE WHEN CAST(@FromDate  AS DATE) = CAST(GETDATE() AS DATE) THEN 0 ELSE eod.Id END
     ...
     -- INNER JOIN Pricelist pl ON pl.Id = odi.PricelistId AND pl.[Type] = 4
