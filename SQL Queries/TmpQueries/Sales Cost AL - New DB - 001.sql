...
FROM OrderDetailInvoices odi
     ...
     INNER JOIN Invoices i ON i.Id = odi.InvoicesId AND
                IsNull(i.IsVoided, 0) = 0 and
                ISNULL(i.EndOfDayId,0) = CASE WHEN CAST(@FromDate  AS DATE) = CAST(GETDATE() AS DATE) THEN 0 ELSE eod.Id END
