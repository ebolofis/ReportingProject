declare @FromDate date = getdate(),  --'2016-04-01',
        @ToDate date = GetDate()     --'2016-04-01',
...
FROM (
     SELECT ProductId, ProductCode , ProductDescription, ProductCategoryCode,
            ...
            Price, Net, VatAmount, FODay, IsExtra, InvoiceType, DepartmentId, PosInfoId

     FROM (
          SELECT --PosInfoDescription = pi.Description,
          ...
          WHERE IsNull(i.IsDeleted, 0) = 0 and
                ...
                IsNull(eod.FODay, CAST(getdate() AS DATE)) BETWEEN CAST(@FromDate AS DATE) AND CAST(@ToDate AS DATE)

          group by i.Id, odi.ProductId, odi.Description, pc.Code, pc.Description, c.Description,
                   pl.Code, odi.VatId, odi.Abbreviation,odi.Counter,
                   i.Id, i.Cover, i.Total, i.Rooms,
                   odi.SalesTypeId, st.Description, IsNull(eod.FODay, CAST(getdate() AS DATE)),
                   IsNull(i.EndOfDayId, 0), IsExtra, InvoiceType, DepartmentId, i.PosInfoId
	) t
...
GROUP BY ProductId, ProductCode, ProductDescription,
         ProductCategoryCode, ProductCategoryDescrtiption, 
         CategoryDescription, FODay, prime.InvoiceType, DepartmentId, PosInfoId
