...
from (
   select ProductId, ProductCode , ProductDescription,
          ...
          Price, Net, VatAmount, FODay,IsExtra, CategoryId, t.PosInfoId, t.PosInfoDescription
   ...
   where IsNull(i.IsDeleted, 0) = 0 and IsNull(odi.IsDeleted, 0) = 0 and InvoiceType not in (2,3,8)
	   ...
	   and odi.Total > 0
   group by i.Id, odi.ProductId,  odi.Description, pc.Id, pc.Description, c.Description,
      ...
      IsExtra, c.Id, i.PosInfoId, pi.Description
   ) t
   ) prime
--where ProductId= 1674
group by ProductId, ProductCode, ProductDescription,
         ProductCategoryId, ProductCategoryDescrtiption, CategoryDescription,
         FODay, CategoryId, PosInfoId, PosInfoDescription
